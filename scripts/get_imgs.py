...
SIZE = (500, 500)
path = '../data/rusmuseum/'
print([int(f) for f in os.listdir(path) if os.path.isdir(path + f) if re.match(r'\d*', f) and int(f) not in range(3)])
for dir in [int(f) for f in os.listdir(path) if
            os.path.isdir(path + f) and re.match(r'\d*', f) and int(f) not in range(3)]:
    print(dir)
    df = pd.read_csv(path + str(dir) + '/index.csv')
    # print(df.head())
    dir_contents = os.listdir(path + f"{str(dir)}")
    print(dir_contents)
    for x in list(df.iterrows()):
        if x[1]["id"] + ".jpeg" in dir_contents:
            print(x[1]["id"])
            continue
        # print(x[1])
        # print(x[1]['url'])
